...
def tower_api(job_id,limit_address,ui_data,towerUrl,headers):
    url = f'{towerUrl}/{job_id}/launch/'
    data = {'extra_vars': {'netgroup': ui_data["NetGroupName"].strip()},"limit": limit_address}
...
def lambda_handler(event, context):
    try:
        ...
        res_get= table.get_item(Key={'uai': ui_data["uai"],'instanceID': ui_data["instanceID"]})        
        ...
        if res_get['Item']['Connectivity'] =='OK':
            ...
            if netgrp_id != -1:
                ...
                invoke_custom_pb=tower_api(netgrp_id,Private_ip,ui_data,towerUrl,headers)
            ...
            if connect_id != -1:
                ...
                invoke_ldap_pb=tower_api(connect_id,Private_ip,ui_data,towerUrl,headers)
